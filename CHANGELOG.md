...
### Added
- **ClawHub Integration** — Built-in ClawHub API client with browse and search capabilities
- **Enhanced Local Skills Management** — Dual-tab design separating Skill Repository and Local Skills
- **Batch Local Skills Import** — Select and import multiple local skills at once
- **Source Filtering** — Filter local skills by their source
- **Session Caching** — Improved performance with session storage caching for scan results
- **Auto-inferred Skill Descriptions** — Automatically extract descriptions during local scan
- **Fresh App Icons** — Updated application icons across all platforms

### Changed
- **Independent Sync Mode per Custom Agent** — Each custom agent can have its own sync mode (symlink/copy) independent of global default
- **Segmented Control UI** — Intuitive toggle interface for sync mode switching in Settings
- **Auto Re-Sync** — Automatically re-sync skills when sync mode is changed
- **Smart Path Conflict Detection** — Improved symlink handling to prevent false positives during file copy
- **Local Skills Scanner Migration** — Moved from Install Skills to My Skills for streamlined workflow
- **Database Schema v5** — Added description field support for enhanced skill metadata

### Fixed
- Fixed symlink path conflict detection to correctly handle symlinks pointing to source directory
- Fixed false positive infinite recursion warnings when target is symlink inside source
